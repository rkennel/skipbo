...
export default abstract class PersonEntityService<
  T extends Person
> extends EntityService<T> {
  validateNewCreation(person: T): T {
    if (!person.gameid) {
      throw new Error(
        `Must specify game for ${this.getEntityName()} to be added to`
      );
    }
  ...
  deleteById(id: string) {
    ...
    if (entity && entity.gameid) {
      game = this.getGameById(entity.gameid);
    }
