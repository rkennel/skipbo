...
export function personEntityTests(entityName: string) {
  ...
  describe("Create and Read Tests", () => {
    ...
    it("Fails to add spectator if a game id is not specified", async () => {
      ...
      const response: Response = await supertest(server.server)
        .post(`/${entityName}`)
        .send(person)
      ...
      expect(response.body.errorMessage).toEqual(
        `Must specify game for ${entityName} to be added to`
      );
    });
    ...
    it("Fails to add spectator if a game id specified does not exist", async () => {
      ...
      expect(response.body.httpStatus).toEqual(400);
      expect(response.body.errorMessage).toEqual(
        `Game: ${person.gameid} does not exist`
      );
    });
...
async function createPersonOnServer(
  server: SkipBoServer,
  entityName: string,
  spectator: Spectator
): Promise<Spectator> {
  const postResponse = await supertest(server.server)
    .post(`/${entityName}`)
    .send(spectator)
    .set("Accept", "application/json");
...
async function cheaterTests(
  ...
  if (performingUpdate) {
    it("Updating game id throws a 400 error", async () => {
      ...
      expect(putResponse.body.errorMessage).toEqual(
        `Cannot update game id via this method`
      );
    });
  }
  ...
  if (entityName === Player.ENTITY_NAME) {
    it("Setting stock pile cards throws a 400 error", async () => {
      ...
      expect(response.body.httpStatus).toEqual(400);
      expect(response.body.errorMessage).toEqual(
        `Cannot set player stockpiles via this method`
      );
    });
    ...
    it("Setting hand cards throws a 400 error", async () => {
      ...
      expect(response.body.httpStatus).toEqual(400);
      expect(response.body.errorMessage).toEqual(
        `Cannot set player hand via this method`
      );
    });
    ...
    it("Setting discard pile cards throws a 400 error", async () => {
      ...
      expect(response.body.httpStatus).toEqual(400);
      expect(response.body.errorMessage).toEqual(
        `Cannot set player discard piles via this method`
      );
    });
  }
}
