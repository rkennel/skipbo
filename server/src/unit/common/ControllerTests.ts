...
export function createAndReadTests(
  server: SkipBoServer,
  entityName: string,
  createEntityFunc?: () => Entity
) {
  it("Before any data has been added, get returns an empty list", async () => {
    const response: Response = await supertest(server.server).get(
      `/${entityName}`
    );
  ...
  it(`I can create a ${entityName}`, async () => {
    const response: Response = await createEntity(
      server,
      entityName,
      createEntityFunc
    );
  ...
  it(`After I create a ${entityName}, getAll returns at least one ${entityName}`, async () => {
    ...
    const getResponse: Response = await supertest(server.server).get(
      `/${entityName}`
    );
...
export function updateTests(
  ...
  entityName: string,
  createEntityFunc: () => Entity,
  updateEntityFunc: (entity: Entity) => Entity
) {
  ...
  it(`I can update a ${entityName} using a put`, async () => {
    ...
    const response: Response = await supertest(server.server)
      .put(`/${entityName}/${updatedEntity.id}`)
      .send(updatedEntity);
    expect(response.status).toEqual(200);
...
export function deleteTests(
  ...
  it(`I can delete ${entityName} that I have created`, async () => {
    ...
    const deleteResponse: Response = await supertest(server.server).delete(
      `/${entityName}/${entityCreated.id}`
    );
